...
export class CouponsService {
  ...
  /**
   * Bulk validation for cart: computes which items are eligible and how many can be applied
   * given global/per-user usage limits. Does not reserve; reservation happens in payment creation.
   */
  async validateCouponForCartItems(
    dto: ValidateBulkCouponDto,
    userId: string,
    ...
    if (!dto.cartItemIds || dto.cartItemIds.length === 0) {
      return {
        ...
        appliedCount: 0,
        totalItems: 0,
        totals: { originalAmount: '0.00', discountAmount: '0.00', finalAmount: '0.00', currency: 'INR' },
        items: [],
    ...
    if (!coupon) {
      return {
        ...
        couponCode: code,
        appliedCount: 0,
        totalItems: dto.cartItemIds.length,
        totals: { originalAmount: '0.00', discountAmount: '0.00', finalAmount: '0.00', currency: 'INR' },
        items: dto.cartItemIds.map((id) => ({
          cartItemId: id,
          itemType: 'COURSE',
          currency: 'INR',
          originalAmount: '0.00',
    ...
    // Restrictions (loaded once)
    const [categoryAssocs, courseAssocs] = await Promise.all([
      this.db.select().from(couponCategories).where(eq(couponCategories.couponId, coupon.couponId)),
      this.db.select().from(couponCourses).where(eq(couponCourses.couponId, coupon.couponId)),
    ]);
    const hasRestrictions = categoryAssocs.length > 0 || courseAssocs.length > 0;
    const restrictedCategoryIds = new Set(categoryAssocs.map((c) => c.categoryId));
    const restrictedCourseIds = new Set(courseAssocs.map((c) => c.courseId));

    // Load cart items with minimal joins (avoid N+1)
    const cart = await this.db.query.cartItems.findMany({
      where: and(eq(cartItems.userId, userId), inArray(cartItems.cartItemId, dto.cartItemIds)),
      with: {
        course: true,
    ...
    if (cart.length !== dto.cartItemIds.length) {
    ...
    const [globalUsage, userUsage] = await Promise.all([
      ...
      this.db
        .select({ count: sql<number>`count(*)::int` })
        .from(couponUsages)
        .where(and(eq(couponUsages.couponId, coupon.couponId), eq(couponUsages.userId, userId), countable)),
    ]);

    const globalUsed = Number(globalUsage?.[0]?.count || 0);
    const userUsed = Number(userUsage?.[0]?.count || 0);

    const globalRemaining = coupon.usageLimit === null ? Number.POSITIVE_INFINITY : Math.max(0, coupon.usageLimit - globalUsed);
    const userRemaining = coupon.usageLimitPerUser === null ? Number.POSITIVE_INFINITY : Math.max(0, coupon.usageLimitPerUser - userUsed);
    let remaining = Math.min(globalRemaining, userRemaining);

    // Preserve requested order
    const byId = new Map(cart.map((c) => [c.cartItemId, c]));
    const ordered = dto.cartItemIds.map((id) => byId.get(id)!);

    const items: BulkCouponItemResult[] = [];
    let appliedCount = 0;
    ...
    const currency = ordered[0]?.currency || 'INR';

    for (const item of ordered) {
      const itemCurrency = item.currency || currency;
      const originalAmount = Number(item.price || 0);
      totalsOriginal += originalAmount;

      const course = item.itemType === 'COURSE' ? item.course : item.section?.course;
      const courseId = course?.courseId || item.courseId || null;
      ...
      if (coupon.minPurchaseAmount && originalAmount < Number(coupon.minPurchaseAmount)) {
        items.push({
          ...
          valid: false,
          reason: 'MIN_AMOUNT_NOT_MET',
          message: `Minimum purchase amount is ${itemCurrency} ${coupon.minPurchaseAmount}`,
        });
        totalsFinal += originalAmount;
        continue;
      }
      ...
      if (hasRestrictions) {
        const courseApplicable = restrictedCourseIds.has(courseId);
        const categoryApplicable = categoryId ? restrictedCategoryIds.has(categoryId) : false;
        if (!courseApplicable && !categoryApplicable) {
          items.push({
            cartItemId: item.cartItemId,
            itemType: item.itemType,
      ...
      if (remaining <= 0) {
        items.push({
          ...
          finalAmount: originalAmount.toFixed(2),
          valid: false,
          reason: globalRemaining <= 0 ? 'USAGE_LIMIT_EXCEEDED' : 'USER_LIMIT_EXCEEDED',
          message:
            globalRemaining <= 0
      ...
      // Compute discount for this item
      let discountAmount: number;
      if (coupon.discountType === 'PERCENTAGE') {
        discountAmount = originalAmount * (Number(coupon.discountValue) / 100);
      ...
      discountAmount = Math.min(discountAmount, originalAmount);
      const finalAmount = Math.max(originalAmount - discountAmount, 0.01);
      ...
      totalsFinal += finalAmount;
    }

    return {
      valid: appliedCount > 0,
      couponCode: code,
      couponId: coupon.couponId,
      appliedCount,
      totalItems: dto.cartItemIds.length,
      totals: {
        originalAmount: totalsOriginal.toFixed(2),
        discountAmount: totalsDiscount.toFixed(2),
        finalAmount: totalsFinal.toFixed(2),
        currency,
      },
      items,
      message:
        appliedCount > 0
          ? `Coupon applied to ${appliedCount} item(s)`
          : 'Coupon is not applicable to any items',
    };
  }
