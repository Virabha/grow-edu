...
function CoursesContent() {
  ...
  const hasFilters = !!debouncedSearch.trim() || status !== "all" || !!categoryId;
          ...
          >
            ...
            <h1 className="font-display mt-3 text-3xl font-medium leading-[1.05] tracking-tight sm:text-4xl md:text-5xl">
              {selectedCategory ? (
                <>
                  Courses in{" "}
                  <em className="text-primary">{selectedCategory.name}</em>
                </>
              ) : (
                <>
                  Browse <em className="text-primary">all courses.</em>
                </>
              )}
            </h1>
            <p className="mx-auto mt-3 max-w-xl text-sm leading-relaxed text-background/80 sm:text-base">
              {selectedCategory?.description ??
                "Explore our full catalogue of batches across every discipline."}
            </p>
    ...
    >
      ...
      <p className="max-w-sm text-sm text-muted-foreground">
        {hasFilters
          ...
          : "Courses will appear here — check back soon."}
      </p>
      {hasFilters && (
